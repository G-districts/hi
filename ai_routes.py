...
def api_classify():
    """Classify a URL using the AI classifier and return its category information.

    IMPORTANT: This endpoint no longer decides blocking on its own.
    The extension uses the returned category together with the
    student's active policy from /api/policy to decide whether
    to block a site.
    """
    ensure_schema()
    body = request.json or {}
